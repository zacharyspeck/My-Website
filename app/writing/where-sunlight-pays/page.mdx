...
export const metadata = {
  ...
  description: 'A thesis on orbital energy: where the value will accrue and why most approaches have failed.',
}
...
In his article, he invents the concept of space-based solar power (SBSP), which entails flying a giant solar panels out to space, having them generate power via the sun and then beaming the electricity down to Earth as microwaves, and a ground antenna catches it and plugs it into the grid. This would allow solar power generation to occur without solar panels, even during the nighttime. 24/7 unlimited energy from the sun.
...
If batteries aren't the perfect solution to maximizing solar panel utilization, then what is? This is a problem that then 25-year-old Ben Nowack toiled with for hours as he tried to ruthlessly imagine solutions to this problem. An avid thinker and creator, Ben had been making YouTube videos where he displayed makeshift bombs, which ultimately led to him getting an engineering gig at SpaceX where he built rockets. Upon leaving SpaceX, he was feeling more entrepreneurial than ever, so he spent many late nights envisioning how he could impact the world.
